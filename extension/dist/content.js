...
function toggleListening() {
    ...
    recognition.onstart = () => {
        ...
        if (micStatus)
            micStatus.innerHTML = "Listening to audience...";
    };
    recognition.onresult = async (event) => {
        const transcript = event.results[0][0].transcript;
        if (micStatus)
            micStatus.innerHTML = `<i>Heard: "${transcript}"</i><br/><span style="color:#60a5fa;">Thinking...</span>`;
        // Send to backend
        try {
            ...
            if (data.hint && data.hint.answer_hint) {
                ...
                if (data.hint.talking_points?.length > 0) {
                    ...
                    data.hint.talking_points.forEach((tp) => {
                        qaHtml += `<li>${tp}</li>`;
                    });
                    qaHtml += `</ul>`;
                }
                ...
                if (micStatus)
                    micStatus.innerHTML = "No clear hint generated.";
            }
        }
        catch (err) {
            console.error(err);
            if (micStatus)
                micStatus.innerHTML = "Error fetching answer hint.";
        }
    };
    recognition.onerror = (event) => {
        ...
        if (micBtn)
            micBtn.textContent = "🎤 Listen";
        if (micStatus)
            micStatus.innerHTML = `Error: ${event.error}`;
    };
    recognition.onend = () => {
